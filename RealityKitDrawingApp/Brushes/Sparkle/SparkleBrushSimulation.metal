...
// Lorenz Attractor Simulation (WORKING VERSION)
[[kernel]]
void sparkleBrushSimulate(device const SparkleBrushParticle *particles [[buffer(0)]],
                          device SparkleBrushParticle *output [[buffer(1)]],
                          constant SparkleBrushSimulationParams &params [[buffer(2)]],
                          uint particleIdx [[thread_position_in_grid]])
{
    ...
    // --- 1. RANDOMNESS & PERSONALITY ---
    // Create a unique random seed for this particle based on its ID
    float2 seed = float2(float(particleIdx), params.deltaTime);
    float randomVal = fract(sin(dot(seed, float2(12.9898, 78.233))) * 43758.5453);
    
    // --- 2. LORENZ CONFIGURATION ---
    const float sigma = 10.0;
    const float rho = 28.0;
    const float beta = 8.0 / 3.0;
    
    // Scale Input: Maps World Meters (small) to Lorenz Math (big)
    // Decreased to 15.0 to prevent "glitching" (math explosions)
    const float scaleInput = 15.0;
    
    // speedFactor: How fast they fly along the path
    const float speedFactor = 0.6;
    
    // --- 3. GET POSITION ---
    // If the particle is at exactly (0,0,0), the Lorenz math returns 0 velocity.
    // We add a tiny offset based on the randomVal to "kick" it into the flow.
    float3 pos = particle.attributes.position;
    if (length(pos) < 0.01) {
        pos += float3(randomVal - 0.5, randomVal - 0.5, randomVal - 0.5) * 0.1;
    }

    // Convert to Lorenz Space
    float3 p = pos * scaleInput;

    // --- 4. CALCULATE LORENZ FORCES ---
    ...
    // --- 5. UPDATE VELOCITY ---
    // Target Velocity is the direction the Lorenz attractor wants to go
    float3 targetVelocity = lorenzForce * speedFactor * 0.1; // Scale down force
    
    // Smoothly blend current velocity towards target (0.1 = 10% blend per frame)
    // This creates the "Curve" instead of jagged jitter
    particle.velocity = mix(particle.velocity, targetVelocity, 0.1);
    
    // Add a tiny bit of random noise to fluff up the volume (so it's not a thin line)
    ...
    // --- 6. SAFETY CLAMP ---
    // If velocity gets too high, the simulation explodes/glitches. Limit it.
